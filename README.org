      ...
      let vdomS =
        S.map(
          ...
          (name) =>
            ...
            <div>
              <input _type="text" onChange=(ReactReact.Utils.emitEventToStream(nameF)) />
              <ShowName name />
            </div>,
          nameS
        );

      let component = ReactReact.reducerComponent("Input");
...
*** Working with side-effects
    ...
    ReasonReact's [[https://reasonml.github.io/reason-react/docs/en/state-actions-reducer.html#important-notes][documentation]] mentions that the ~reducer~ function
    should be pure. That's not exactly a problem with ReactReact's
    streams (since ~reduce~ is always called with the result of the
    signal, it does not call the signal itself).

    I recommend you to still be as pure as possible with your streams.
    If you want to debug a stream or anything similar, you can always
    subscribe to it with a ~signal(unit)~ stream, like the following
...
*** Using the result of promises
    ...
    convert promises to events. If you're using a library that return
    ...
    promises, and eventually even create components out of these signals.

    It uses ~Js.Result.t~ because every promise might fail and throw
